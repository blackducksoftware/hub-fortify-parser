...
public class BlackDuckCSVParser {
     ...
     *
     * @param inputStream stream that contains uploaded BlackDuck scan result file data.
    ...
    public BlackDuckScan parseScan(InputStream inputStream) throws IOException {
        /*
          ...
          We were informed that BlackDuc hub supports another export format (JSON). If JASON file contains any information
          about scan, it should be better to re-implement the parsing logic and move from CSV parsing to JSON fiel parsing.
        ...
        */
        ...
        /*
          ...
          All parser plugins that SSC currently supports store the scan date inside the scan result file. You will need to
          figure out where to get this date from black duck scan results.
          This date also helps SSC to understand the status of issue - is it new, or is it reintroduced, or is it fixed.
          If there is not way to get this file from CSV file, scan date must be included in scan.info file. See the comment
          above for more details.
        */
        final Date scanDate = new Date();
     ...
     * @param inputStream stream that contains uploaded BlackDuck scan result file data.
     * @param beanProcessor BeanProcessor implementation that is responsible for fetching vulnerabilities from the
     * rows of the provided black duck result CSV file.
     * @throws ScanParsingException
     * @throws IOException If an I/O error occurs during CSV file processing.
     * @throws IllegalArgumentException if inputStream and / or beanProcessor are null.
     */
